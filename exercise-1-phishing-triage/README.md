...
```bash
cd "C:\Users\alexm\SOAR Excercises\exercise-1-phishing-triage"
python playbook.py
```

This runs two demo scenarios (low-risk sender with no attachment, and malicious sender with suspicious hash) and prints the war room summaries.

### Run tests

```bash
python -m pytest test_playbook.py -v
```

Or without pytest:

```bash
python -m unittest test_playbook -v
```
